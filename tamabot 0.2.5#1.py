...
async def on_message(message):
    ...
    if a == 0:
        if user_message.lower() == 'hi':
            ...
            await message.channel.send(f'your mum!')
            return
...
async def status_cycle():
    ...
    statuses = [
        f"on {len(client.guilds)} servers | >help","your dad"]
...
async def changelog(ctx):
    embed = discord.Embed(title = "Changelog",
                          ...
                          description = "Version v0.2.5#1 Patch Notes:\n- Added a new FUN command: caught")
    await ctx.send(embed=embed)
